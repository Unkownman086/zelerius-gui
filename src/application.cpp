...
{
    setApplicationName("zelerius"); // do not change becasuse it also changes data directory under Mac and Win
    setApplicationDisplayName(tr("Zelerius Wallet"));
...
bool WalletApplication::init()
{
    ...
    QString path = dataDir.absoluteFilePath("zelerius-gui.lock");
    m_lockFile.reset(new QLockFile(path));

    if (!m_lockFile->tryLock())
    {
        WalletLogger::warning(tr("[Application] Zelerius wallet GUI already running"));
        QMessageBox::warning(nullptr, QObject::tr("Error"), tr("Zelerius wallet GUI already running"));
        return false;
    }
